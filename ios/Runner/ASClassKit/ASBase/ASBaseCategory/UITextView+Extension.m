...
- (void)drawRect:(CGRect)rect {
    ...
    if((self.ug_maximumLimit > 0) && (self.ug_characterLengthPrompt == YES)) {
        ...
        //        [limitStr drawInRect:CGRectMake(x, rect.size.height-20+self.contentOffset.y, width, 18) withAttributes:maximumLimitAttributes];
        [limitStr drawInRect:CGRectMake(x, 0, width, 18) withAttributes:maximumLimitAttributes];
    }
...
- (void)characterTruncation {
    ...
    if((self.textHandle) && (![self.text isEqualToString:STRING(self.lastTextStr)])) {
