//
//  ASBaseCollectionView.m
//  AS
//
//  Created by SA on 2025/4/11.
//

#import "ASBaseCollectionView.h"

@implementation ASBaseCollectionView
...
- (instancetype)initWithFrame:(CGRect)frame collectionViewLayout:(nonnull UICollectionViewLayout *)layout {
    ...
    if (self) {
        ...
        self.emptyType = kCollectionViewEmptyDefault;
    }
    return self;
}

- (void)setEmptyType:(CollectionViewEmptyType)emptyType {
    ...
    self.buttonText = @"";
    switch (emptyType) {
        case kCollectionViewEmptyDefault: {
            self.imageURL = @"";
        }
            break;
        case kCollectionViewEmptyNoData:
        {
            self.imageURL = @"empty_common";
            self.textTitle = @"暂无数据";
        }
            break;
        case kCollectionViewEmptyLoadFail:
        {
            self.imageURL = @"empty_fial";
            self.textTitle = @"加载失败";
        }
            break;
        case kCollectionViewEmptyVideo:
        {
            self.imageURL = @"empty_video";
            self.textTitle = @"暂无视频秀";
        }
            break;
        case kCollectionViewEmptySearch:
        {
