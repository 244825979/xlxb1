...
- (void)imageTaken {
    if (self.imagePickerVc.cameraDevice == UIImagePickerControllerCameraDeviceFront) { //如果是自拍则翻转预览图
        NSMutableArray *subviews = [NSMutableArray arrayWithObject:self.imagePickerVc.view];
        while ([subviews count] != 0) {
            UIView *subview = [subviews firstObject];
            [subviews removeObjectAtIndex:0];
            [subviews addObjectsFromArray:subview.subviews];
            if ([subview isKindOfClass:[UIImageView class]]) {
                subview.transform = CGAffineTransformScale(self.imagePickerVc.cameraViewTransform, -1, 1);
            }
        }
    }
}
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (model.type == TZAssetModelMediaTypeVideo && !tzImagePickerVc.allowPickingMultipleVideo) {
        if (tzImagePickerVc.selectedModels.count > 0) {
            ...
            //判断设置的时间不等于0，且设置了最小录制时长。时间小于设置的时长，就提示且禁止跳转
            if (model.asset.duration < tzImagePickerVc.videoMinSelectDuration &&
                tzImagePickerVc.videoMinSelectDuration != 0) {
                if (tzImagePickerVc.viewBackBlock) {
                    tzImagePickerVc.viewBackBlock(1);
                }
                return;
            }
...
- (void)pushImagePickerController {
    ...
    if ([UIImagePickerController isSourceTypeAvailable: sourceType]) {
        ...
        //新增是否是自拍照
        if (tzImagePickerVc.isSelfieCamera == YES) {
...
- (void)imagePickerController:(UIImagePickerController*)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    if ([type isEqualToString:@"public.image"]) {
        ...
        if (self.imagePickerVc.cameraDevice == UIImagePickerControllerCameraDeviceFront) {
                   //如果是自拍，就使用镜面翻转图片
                   UIImage* flippedImage = [UIImage imageWithCGImage:photo.CGImage
                                                               scale:photo.scale
                                                         orientation:UIImageOrientationLeftMirrored];  //镜面翻转图片
                   
                   if (flippedImage) {
                       self.isSavingMedia = YES;
                       [[TZImageManager manager] savePhotoWithImage:flippedImage meta:nil location:self.location completion:^(PHAsset *asset, NSError *error){
        ...
        //判断设置的时间不等于0，且设置了最小录制时长。时间小于设置的时长，就提示且禁止跳转
        if (second < imagePickerVc.videoMinSelectDuration &&
            imagePickerVc.videoMinSelectDuration != 0) {
            if (imagePickerVc.viewBackBlock) {
                imagePickerVc.viewBackBlock(1);
            }
            return;
        }
        [imagePickerVc showProgressHUD];
