...
/// 自定义新增属性：是否是自拍，默认为NO
@property(nonatomic, assign) BOOL isSelfieCamera;

/// 自定义新增属性：默认为0不限制，视频选中最小时长
@property(nonatomic, assign) NSTimeInterval videoMinSelectDuration;

/// 自定义新增属性：可自定义回调时间 tag回调类型,1表示视频时长不够提醒
@property (nonatomic, copy) void(^viewBackBlock)(NSInteger tag);
