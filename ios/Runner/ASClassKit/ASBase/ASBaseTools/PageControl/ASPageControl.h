...
typedef NS_ENUM(NSUInteger,ASPageControlType){
    PageControlMiddle = 0, //
    PageControlRight, //
    PageControlLeft //
};
...
@class ASPageControl;
@protocol ASPageControlDelegate <NSObject>
@optional
/// 点击PageControl点的时候,响应事件回调
/// @param pageControl 点击的pageControl
/// @param clickIndex 点击点的下标,从0开始
- (void)xh_PageControlClick:(ASPageControl*)pageControl index:(NSInteger)clickIndex;
@end

@interface ASPageControl : UIControl
/// 代理
@property (nonatomic, weak)     id<ASPageControlDelegate > delegate;
/// Block点击回调
@property (nonatomic, copy)     void (^clickPointBlock)(NSInteger index);
/// 控件位置,默认中间:PageControlMiddle
@property (nonatomic, assign)   ASPageControlType pageAliment;
/// 当前点的大小默认:(6,6)
