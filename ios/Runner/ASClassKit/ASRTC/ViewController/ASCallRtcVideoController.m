...
@interface ASCallRtcVideoController ()<NERtcCallKitDelegate, FUCaptureCameraDelegate, ASWebSocketDelegate>
...
@property (nonatomic, strong) UIImageView *bigCameraCloseIcon;//摄像头关闭的视频Icon图标
@property (nonatomic, strong) UILabel *bigCameraCloseText;//摄像头关闭的提示文案
...
@property (nonatomic, strong) ASAnswerRiskView *riskHintView;//防骗提醒view
...
@property (nonatomic, strong) ASVideoShowPlayerView *videoShowPlayView;//视频秀来电播放
/**数据**/
@property (nonatomic, strong) FUCaptureCamera *mCamera;//美颜相机采集
@end

@implementation ASCallRtcVideoController
...
- (void)createUI {
    ...
    if (self.callModel.is_video_show == 1 && USER_INFO.gender == 2) {
        ...
        [NERtcCallKit.sharedInstance setupLocalView:self.bgVideoView];//本地预览画面
    }
    ...
    [self.itemBg mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.callUserView.mas_bottom);
...
- (void)callInit {
    [NERtcCallKit.sharedInstance addDelegate:self];//执行设置回调监听
    [NERtcCallKit.sharedInstance enableLocalVideo:YES];
    NERtcCallKit.sharedInstance.timeOutSeconds = 60;// 单位为秒
    NSMutableDictionary *parameters = [NSMutableDictionary dictionaryWithDictionary:@{
        @"roomId": STRING(self.callModel.room_id),
        @"fromUid": STRING(self.callModel.from_uid),
        @"fromName": STRING(self.callModel.from_nickname)
    }];
    [parameters setValue:[NSNumber numberWithBool:NO] forKey:@"isMatch"];
    [NERtcCallKit.sharedInstance call:self.callModel.to_uid
                                 type:NERtcCallTypeVideo
                           attachment:[ASCommonFunc convertNSDictionaryToJsonString:parameters]
                          globalExtra:nil
                            withToken:nil
                          channelName:self.callModel.room_id completion:^(NSError * _Nullable error) {
        if (!error) {
            [[ASRtcCallRingManager shared] play];//开启来电铃声
        }
    }];
    //配置外部数据源输入
...
//美颜设置初始化
- (void)setFU {
    NSNumber *isOpenFU = [ASUserDefaults valueForKey:@"isOpenFaceUnity"];//获取保存在本地的美颜开启状态
...
- (void)requestData {
    ...
    if (kStringIsEmpty(self.moneyText) && USER_INFO.gender == 2) {
        [ASRtcRequest requestGetPriceWithUserID:self.callModel.to_uid success:^(ASRtcAnchorPriceModel * _Nullable priceModel) {
...
- (void)answerRequest {
    kWeakSelf(self);
    [ASCommonRequest requestCallRiskWithSuccess:^(ASCallAnswerRiskModel * _Nullable data) {
        if (data.labelList.count > 0) {
            wself.riskHintView.hidden = NO;
            wself.riskHintView.model = data;
        }
    } errorBack:^(NSInteger code, NSString *msg) {
...
- (void)onUserBusy:(NSString *)userID {
    ASLog(@"我是发送方：用户忙线 userID = %@", userID);
    [self closeWithCompletion:^{
        kShowToast(@"用户忙~");
    }];
}
...
- (void)onCameraAvailable:(BOOL)available userID:(NSString *)userID {
    ASLog(@"我是发送方：启用/禁用相机 available = %d", available);
    if (available == NO) {
...
- (void)onFirstVideoFrameDecoded:(NSString *)userID width:(uint32_t)width height:(uint32_t)height {
    ASLog(@"我是发送方：首帧解码成功的回调 userID = %@", userID);
    [NERtcCallKit.sharedInstance setupRemoteView:self.bgVideoView forUser:userID];//调用如下接口设置远端画面
}
...
- (ASCallVideoItemView *)itemBg {
    if (!_itemBg) {
        ...
        _itemBg.itemType = kItemTypeCallNoCalling;
        kWeakSelf(self);
        ...
        _itemBg.cameraSwitchBlock = ^(BOOL isOpen) {
            if (isOpen == YES) {
                if (NERtcCallKit.sharedInstance.callStatus == NERtcCallStatusCalling ||
                    NERtcCallKit.sharedInstance.callStatus == NERtcCallStatusCalled) {//正在呼叫中
                    wself.bigCameraView.hidden = YES;
                } else {
                    wself.smallCameraView.hidden = YES;
                }
            } else {
                if (NERtcCallKit.sharedInstance.callStatus == NERtcCallStatusCalling) {//未接听
        ...
        _itemBg.sendGiftBlock = ^{
            [ASCommonRequest requestGiftTitleSuccess:^(id  _Nullable data) {
                [ASAlertViewManager popGiftViewWithTitles:data toUserID:wself.callModel.to_uid giftType:kSendGiftTypeCall sendBlock:^(NSString * _Nonnull giftID, NSInteger giftCount, NSString * _Nonnull giftTypeID) {
