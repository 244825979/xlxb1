//
//  ASWithdrawRecordListController.m
//  AS
//
...
@interface ASWithdrawRecordListController ()<UITableViewDelegate, UITableViewDataSource>
...
@property (nonatomic, copy) NSString *createDayTime;//创建时间，当天时间+当天收入
@end

@implementation ASWithdrawRecordListController
...
- (void)requestList {
    ...
    [ASPayRequest requestEarningsListWithPage:self.page date:self.date category:category newType:newType success:^(id  _Nullable data) {
        ...
        for (int i = 0; i < array.count; i++) {
            ...
            if (!kStringIsEmpty(model.date)) {
                if (![model.date isEqualToString:wself.createDayTime]) {
                    wself.createDayTime = model.date;
                    [array insertObject:[NSString stringWithFormat:@"%@,%@",model.date, model.day_income] atIndex:i];
                }
            } else {
                NSString *create_time = [model.create_time substringToIndex:7];
                if (![create_time isEqualToString:wself.createDayTime]) {
                    wself.createDayTime = create_time;
