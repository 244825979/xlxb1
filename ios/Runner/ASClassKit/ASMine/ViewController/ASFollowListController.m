...
@interface ASFollowListController ()<UITableViewDelegate, UITableViewDataSource>
...
@implementation ASFollowListController
...
- (void)requestList {
    kWeakSelf(self);
    [ASMineRequest requestFollowListWithPage:self.page success:^(id  _Nullable data) {
        NSArray *array = data;
        if(wself.page == 1){
...
- (ASBaseTableView *)tableView {
    if (!_tableView) {
        ...
        _tableView.emptyType = kTableViewEmptyGuanzhu;
    }
    return _tableView;
}
@end
