...
open class MessageContentModel: NSObject, MessageModel {
    ...
    public var remoteHeight: CGFloat = 0//扩展内容的高度
    public var remoteData: [String: AnyObject] = [:]
        ...
        didSet {
            if isRevoked {
                ...
                if let isSend = message?.isOutgoingMsg, isSend, message?.messageType == .text, timeOut == false {
                    contentSize = CGSize(width: 218, height: 28)
                } else {
                    contentSize = CGSize(width: 130, height: 28)
                }
    ...
    public required init(message: NIMMessage?) {
        ...
        if (message?.isOutgoingMsg == false) {
            //接收的消息
            if let remoteExt = message?.remoteExt as? [String: AnyObject] {
                if let isCut = remoteExt["is_cut"] as? NSInteger {
                    print("判断是否有来消息提示金币或者会员等信息提示：remoteExt = \(remoteExt), type = \(type)")
                    if isCut == 1 {
                        remoteHeight = 30.0
                        remoteData = remoteExt
                    } else {
                        if let isVip = remoteExt["vip"] as? NSInteger, isVip == 1 {
                            remoteHeight = 30.0
                            remoteData = remoteExt
                        } else {
                            remoteHeight = 0.0
