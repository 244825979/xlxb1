...
open class NEBaseChatMessageCell: NEChatBaseCell {
    ...
    // 已读未读视图
    public var readView = CirleProgressView(frame: CGRect(x: 0, y: 0, width: 25, height: 18))
    ...
    public var messageCostHintView = QWIMMessageCostHintView() // 价格提醒
    ...
    open func initProperty() {
        ...
        //        bubbleImage
        bubbleImageLeft.backgroundColor = UIColor(hexString: "#FFFFFF")
...
//            .resizableImage(withCapInsets: NEKitChatConfig.shared.ui.messageProperties.backgroundImageCapInsets)
        ...
        bubbleImageRight.backgroundColor = UIColor(hexString: "#FF1832")
...
//            .resizableImage(withCapInsets: NEKitChatConfig.shared.ui.messageProperties.backgroundImageCapInsets)
    ...
    open func baseCommonUILeft() {
        ...
        avatarImageLeftAnchor = avatarImageLeft.leftAnchor.constraint(equalTo: contentView.leftAnchor, constant: 14)
        NSLayoutConstraint.activate([
            avatarImageLeft.topAnchor.constraint(equalTo: timeLabel.bottomAnchor, constant: 6),
            avatarImageLeftAnchor!,
            avatarImageLeft.widthAnchor.constraint(equalToConstant: 42),
            avatarImageLeft.heightAnchor.constraint(equalToConstant: 42),
        ])
        ...
        NSLayoutConstraint.activate([
            ...
            fullNameLabel.rightAnchor.constraint(equalTo: contentView.rightAnchor, constant: -14),
        ...
        bubbleTopAnchorLeft = bubbleImageLeft.topAnchor.constraint(equalTo: avatarImageLeft.topAnchor, constant: 2)
        ...
        messageCostHintView.translatesAutoresizingMaskIntoConstraints = false
        messageCostHintView.isHidden = true
        contentView.addSubview(messageCostHintView)
        NSLayoutConstraint.activate([
            messageCostHintView.leftAnchor.constraint(equalTo: bubbleImageLeft.leftAnchor),
            messageCostHintView.topAnchor.constraint(equalTo: bubbleImageLeft.bottomAnchor),
            messageCostHintView.heightAnchor.constraint(equalToConstant: 30),
        ])
    }
    ...
    open func baseCommonUIRight() {
        ...
        NSLayoutConstraint.activate([
            avatarImageRight.rightAnchor.constraint(equalTo: contentView.rightAnchor, constant: -14),
            avatarImageRight.widthAnchor.constraint(equalToConstant: 42),
            avatarImageRight.heightAnchor.constraint(equalToConstant: 42),
        ...
        NSLayoutConstraint.activate([
            bubbleImageRight.topAnchor.constraint(equalTo: avatarImageRight.topAnchor, constant: 3),
        ...
        NSLayoutConstraint.activate([
            ...
            readView.centerYAnchor.constraint(equalTo: bubbleImageRight.centerYAnchor),
            readView.widthAnchor.constraint(equalToConstant: 25),
            readView.heightAnchor.constraint(equalToConstant: 18),
        ])
    ...
    open func setSelect(_ model: MessageContentModel, _ enableSelect: Bool = false) {
        ...
        avatarImageLeftAnchor?.constant = enableSelect ? 42 : 14
    }
    ...
    override open func setModel(_ model: MessageContentModel, _ isSend: Bool) {
        ...
        if var avatarURL = model.avatar, !avatarURL.isEmpty {
            avatarURL = NEKitChatConfig.shared.imageURL + avatarURL
            avatarImage
        ...
        if model.fullNameHeight > 0 {
            ...
            bubbleTopAnchorLeft?.constant = 3
        }
        ...
        if isSend, model.message?.deliveryState == .deliveried {
            if model.message?.session?.sessionType == .P2P {
                if let messageModel = model.message, messageModel.messageType == .text || messageModel.messageType == .image || messageModel.messageType == .audio {
        ...
        if model.remoteHeight > 0 {
            messageCostHintView.isHidden = false
            messageCostHintView.setData(model.remoteData)
        } else {
            messageCostHintView.isHidden = true
        }
    }
