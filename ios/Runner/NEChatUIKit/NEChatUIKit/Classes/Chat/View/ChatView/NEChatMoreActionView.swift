...
open class NEChatMoreActionView: UIView {
    ...
    func setupConstraints() {
        ...
        collcetionView.frame = CGRect(x: 4, y: 16, width: kScreenWidth - 8, height: 72)
        if rowCount > 1 {
            // 设置行间距
            moreFlowLayout?.minimumInteritemSpacing = 10
        }
...
extension NEChatMoreActionView: UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
    ...
    open func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        var itemModel = self.data?[indexPath.row]
//
