...
open class NEEmotionTool: NSObject {
    ...
    public class func getPrefixAttWithStr(str: String, 
                                          prefixText: String,
                                          font: UIFont,
                             _ offset: CGPoint = CGPoint(x: 0, y: -3)) -> NSMutableAttributedString {
        let regular = "\\[[^\\[|^\\]]+\\]"
        ...
        //添加视频秀显示文案的红色标记
        attStr.addAttribute(.foregroundColor, value: NEConstant.hexRGB(0xF55285), range: NSRange(location:0, length:prefixText.count))
    ...
    public class func getAttWithStr(str: String, font: UIFont, color: UIColor, _ offset: CGPoint = CGPoint(x: 0, y: -3)) -> NSMutableAttributedString {
    ...
    //扩展一个新增上下间距的富文本
    public class func getAttWithStr(str: String, font: UIFont, lineSpacing: CGFloat, color: UIColor, _ offset: CGPoint = CGPoint(x: 0, y: -3)) -> NSMutableAttributedString {
        let att = getAttWithStr(str: str, font: font, offset)
        let style = NSMutableParagraphStyle()
        style.lineSpacing = lineSpacing
        style.lineBreakMode = .byTruncatingTail
        att.addAttribute(.foregroundColor, value: color, range: NSRange(location: 0, length: att.length))
        att.addAttribute(.paragraphStyle, value: style, range: NSRange(location: 0, length: att.length))
        return att
    }
    
    public class func getAttWithEmotion(emotion: NIMInputEmoticon, font: UIFont,
