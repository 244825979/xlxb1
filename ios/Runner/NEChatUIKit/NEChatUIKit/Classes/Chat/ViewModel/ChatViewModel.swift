...
open class ChatViewModel: NSObject, ChatRepoMessageDelegate, NIMChatManagerDelegate,
    ...
    public var isOpenHiding: Bool = false
    ...
    //优化消息服务器校验后，使用此方法发送文本消息，目前支持文本、图片、语音消息
    public func verifySendMessage(message: NIMMessage, _ completion: @escaping (Error?) -> Void) {
        repo.sendMessage(
            message: message,
            session: session,
            completion
    ...
    open func sendTextMessage(text: String, remoteExt: [String: Any]?, _ completion: @escaping (Error?) -> Void) {
        ...
        repo.sendMessage(
            message: MessageUtils.textMessage(text: text, remoteExt: remoteExt, session: session, myUserID: ""),
            session: session,
            completion
        )
    }
    ...
    open func sendTextMessage(text: String, _ completion: @escaping (Error?) -> Void) {
        ...
        repo.sendMessage(
            message: MessageUtils.textMessage(text: text, session: session),
            session: session,
            completion
        )
    }
    ...
    open func sendAudioMessage(filePath: String, _ completion: @escaping (Error?) -> Void) {
        ...
        repo.sendMessage(
            message: MessageUtils.audioMessage(filePath: filePath, session: session, myUserID: ""),
            session: session,
            completion
        )
    }
    ...
    open func sendImageMessage(image: UIImage, _ completion: @escaping (Error?) -> Void) {
        ...
        repo.sendMessage(
            message: MessageUtils.imageMessage(image: image, session: session, myUserID: ""),
            session: session,
            completion
        )
    }
    ...
    open func sendImageMessage(data: Data, ext: String, _ completion: @escaping (Error?) -> Void) {
        ...
        repo.sendMessage(
            message: MessageUtils.imageMessage(data: data, ext: ext, session: session),
            session: session,
            completion
        )
    }
    ...
    open func sendImageMessage(path: String, _ completion: @escaping (Error?) -> Void) {
        ...
        repo.sendMessage(
            message: MessageUtils.imageMessage(path: path, session: session),
            session: session,
            completion
        )
    }
...
//                    OperationItem.selectItem(),
        ...
        case .text:
            items.append(OperationItem.copyItem())
            if isRecall && model?.message?.deliveryState == .deliveried {//如果发送时间小于120秒，允许撤回
