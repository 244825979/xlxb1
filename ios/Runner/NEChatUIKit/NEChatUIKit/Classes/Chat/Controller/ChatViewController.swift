 ...
 */
@objc
public enum VerifyMessageType: NSInteger{
    case text = 1//文本
    case audio = 2//语音
    case image = 3//图片
}
...
@objc
public enum ChatTabbarItemType: Int {
    case emoji = 0
    case gift
    case call
    case expressions
...
//回调
public protocol IMP2PChatDelegate: AnyObject {
    func sendMessage(message: NIMMessage, type: NSInteger)//发送消息
    func buttonClikedItem(type: ChatTabbarItemType)//0表情 1礼物（a面是常用语） 2拨打电话 3常用语 4相册
    func tapUserHeader(isLeft: Bool)//点击头像
    func nextBtnCliked(userID: String)//下一个用户
    func playAudio(url: String)//播放音频
    func stopAudio()//关闭音频
    func isPlayingAudio() -> Bool//是否在播放中
    func didTapCustomCell(customModel: IMCustomDataModel, type: NSInteger)//自定义消息点击。type：自定义消息类型
    func moreViewClikedItem(type: NEMoreActionType)//更多view点击
    func openHidingAction()//如有开启隐身模式执行
}
...
open class ChatViewController: ChatBaseViewController, UINavigationControllerDelegate,
    ...
    private var playingUrl: String?//语音播放URL
    ...
    public var bottomExanpndHeight: CGFloat = 194 // 底部展开高度
    public var bottomMoreViewHeight: CGFloat = 90 // 底部展开moreView高度
    public var normalInputHeight: CGFloat = 104
    ...
    //新增字段
    public weak var p2pChatDelegate: IMP2PChatDelegate?
    public var textHeight: CGFloat = 0.0 //输入文本的高度
    public var bottomViewTopAnchor: NSLayoutConstraint?//inputView的顶部约束
    public var bottomViewHeightAnchor: NSLayoutConstraint?//inputView的高度约束
    public var isPopKeyboard = false //是否弹出键盘
    public var keyboardHeight: CGFloat = 0.0 //键盘的高度
    public var currentInsInBottom = true//标记是否在最底部记录已读未读
    
    public var tableViewBottomConstraint: NSLayoutConstraint?
    public var usefulExpressionsView = QWChatUsefulExpressionsView()
    public var usefulExpressionsHeightConstraint: NSLayoutConstraint?
    public var usefulExpressionsViewHeight: CGFloat = 48.0
    public var usefulExpressionsTitles: [[String: Any]?] = [] {//常用语
        didSet {
            usefulExpressionsView.titles = usefulExpressionsTitles
            if usefulExpressionsTitles.count > 0 {
                usefulExpressionsViewHeight = 48.0
            } else {
                usefulExpressionsViewHeight = 0.1
            }
            //更新约束
            tableViewBottomConstraint?.constant = -usefulExpressionsViewHeight
            usefulExpressionsHeightConstraint?.constant = usefulExpressionsViewHeight
            //滚动最底部
    ...
    public lazy var nextBtn: UIButton = {
        let button = UIButton()
        button.setImage(UIImage.ne_imageNamed(name: "chat_moreuser"), for: .normal)
    ...
    //是否有下一页未读
    public var unreadSessions: [NIMRecentSession] = [] {
        didSet {
            nextBtn.isHidden = unreadSessions.count > 0 ? false : true
        }
    }
    ...
    override open func viewWillAppear(_ animated: Bool) {
        ...
        NEChatDetectNetworkTool.shareInstance.netWorkReachability { [weak self] status in
            if status == .notReachable {
                ...
                NotificationCenter.default.post(name: Notification.Name("netWorkChatReachabilityNotify"), object: 0)
            } else {
                self?.brokenNetworkView.isHidden = true
                self?.contentViewTopAnchor?.constant = 0
                NotificationCenter.default.post(name: Notification.Name("netWorkChatReachabilityNotify"), object: 1)
            }
        }
    }
    ...
    open func commonUI() {
        ...
        if isSystemUser == true {
            bottomViewTopAnchor = bottomView.topAnchor.constraint(equalTo: view.bottomAnchor)
        } else {
            bottomViewTopAnchor = bottomView.topAnchor.constraint(equalTo: view.bottomAnchor, constant: -normalInputHeight - kTabbarBottomHeight)
        }
        ...
        //自定义cell
        cellRegisterDic["11"] = QWChatSystemCustomCell.self//系统cell
        cellRegisterDic["95"] = QWChatSystemCustomCell.self//客服小助手cell
        cellRegisterDic["99"] = QWChatActivityCustomCell.self//活动小助手cell
        cellRegisterDic["14"] = QWChatMessageGiftCustomCell.self//礼物cell
        cellRegisterDic["15"] = QWCallStateCustomCell.self//视频通话状态
        cellRegisterDic["73"] = QWCallStateCustomCell.self//语音通话状态
        cellRegisterDic["72"] = QWChatRecommendCustomCell.self//小助手
        cellRegisterDic["110"] = QWChatRecommendCustomCell.self//一键搭讪
        cellRegisterDic["100"] = QWChatImageCustomCell.self//图片链接
        cellRegisterDic["101"] = QWChatAudioCustomCell.self//语音链接
        cellRegisterDic["203"] = QWChatTextCustomCell.self//自定义文本消息
        //registerCellDic["999"] = ChatMessageTextTipCell.self//提示cell，与time类型的cell分开处理
    ...
    public func nextEvent(button: UIButton) {
        let session = unreadSessions[0]
        if let sessionId = session.session?.sessionId {
            p2pChatDelegate?.nextBtnCliked(userID: sessionId)
        }
    }
    ...
    open func didLongTouchMessageView(_ cell: UITableViewCell, _ model: MessageContentModel?) {
        ...
        if chatInputView.textView.isFirstResponder {
            chatInputView.textView.resignFirstResponder()
            layoutInputView(offset: kTabbarBottomHeight)
        }
        
        operationView?.removeFromSuperview()
        
        // get operations
        guard let items = viewmodel.avalibleOperationsForMessage(model), items.count > 0 else {
            return
        }
    ...
    public lazy var contentView: UIView = {
        ...
        //常用语
        usefulExpressionsView.translatesAutoresizingMaskIntoConstraints = false
        usefulExpressionsView.backgroundColor = .clear
        usefulExpressionsView.clikeBackBlock = { [weak self] text in
            guard let self = self else { return }
            if self.isOpenHiding == true {
                p2pChatDelegate?.openHidingAction()
            } else {
                //回调接口校验当前发送消息
                p2pChatDelegate?.sendMessage(message: MessageUtils.textMessage(text: text, remoteExt: nil, session: viewmodel.session, myUserID: myUserID), type: VerifyMessageType.text.rawValue)
            }
        }
        content.addSubview(usefulExpressionsView)
        NSLayoutConstraint.activate([
            usefulExpressionsView.bottomAnchor.constraint(equalTo: content.bottomAnchor),
            usefulExpressionsView.leftAnchor.constraint(equalTo: content.leftAnchor),
            usefulExpressionsView.rightAnchor.constraint(equalTo: content.rightAnchor)
        ])
        usefulExpressionsHeightConstraint = usefulExpressionsView.heightAnchor.constraint(equalToConstant: usefulExpressionsViewHeight)
        usefulExpressionsHeightConstraint?.isActive = true
        ...
        tableViewBottomConstraint = tableView.bottomAnchor.constraint(equalTo: content.bottomAnchor, constant: 0)
        tableViewBottomConstraint?.isActive = true
        
        //下一个聊天页
        content.addSubview(nextBtn)
        NSLayoutConstraint.activate([
            nextBtn.bottomAnchor.constraint(equalTo: usefulExpressionsView.topAnchor, constant: -15),
            nextBtn.rightAnchor.constraint(equalTo: content.rightAnchor, constant: -15),
            nextBtn.widthAnchor.constraint(equalToConstant: 36),
            nextBtn.heightAnchor.constraint(equalToConstant: 26)
        ])
        return content
    }()
    ...
    func addObseve() {
        ...
        //更新导航的亲密值
        NotificationCenter.default.addObserver(self, selector: #selector(updataIntimateValue(_:)), name: NSNotification.Name(rawValue: "updataIntimateValueNitification"), object: nil)
        //语音播放结束
        NotificationCenter.default.addObserver(self,
                                               selector: #selector(audioPlayCompletionNotifiction(_:)),
                                               name: NSNotification.Name(rawValue: "AudioPlayCompletionNotifiction"),
                                               object: nil)
        ...
        //进入前台--------由另一个程序回到当前程序应用
        NotificationCenter.default.addObserver(self, selector: #selector(applicationFrontDesk), name: UIApplication.didBecomeActiveNotification, object: nil)
    }
    
    //更新导航的亲密值
    @objc private func updataIntimateValue(_ notify: NSNotification) {
        //亲密度
        let myUserID = UserDefaults.standard.value(forKey: "userinfo_user_id") as? String ?? ""
        let intimateKey = myUserID + "_intimate_" + viewmodel.session.sessionId
        let intimateData = UserDefaults.standard.value(forKey: intimateKey) as? NSDictionary ?? [:]
        let score = intimateData["score"] as? NSString ?? ""
        intimacyText.setTitle("\(score)℃", for: .normal)
    }
    ...
    open func keyBoardWillHide(_ notification: Notification) {
        ...
        layoutInputViewWithAnimation(offset: kTabbarBottomHeight, animationDuration)
    }
    ...
    private func scrollTableViewToBottom() {
        ...
        if (tableCount < messageCount && messageCount > 0) {
            //异常数据返回
            print("聊天--获取数据异常返回")
            return
        }
    ...
    open func layoutInputViewWithAnimation(offset: CGFloat, _ animation: CGFloat = 0.3) {
        ...
        if chatInputView.currentType == .audio {
            if !chatInputView.voiceButton.isSelected {
    ...
    func sendContentText(text: String?, attribute: NSAttributedString?) {
        ...
        if viewmodel.isReplying, let msg = viewmodel.operationModel?.message {
            viewmodel.replyMessageWithoutThread(message: MessageUtils.textMessage(text: content, remoteExt: remoteExt, session: viewmodel.session, myUserID: myUserID), target: msg) { [weak self] error in
                NELog.infoLog(
            ...
            if self.isOpenHiding == true {
                ...
                //发送文本消息，进行服务器校验
                p2pChatDelegate?.sendMessage(message: MessageUtils.textMessage(text: content, remoteExt: remoteExt, session: viewmodel.session, myUserID: myUserID), type: VerifyMessageType.text.rawValue)
            }
    ...
    func checkAndSendTypingState() {
        if chatInputView.chatInpuMode == .normal {
            if let content = chatInputView.textView.text, content.count > 0 {
                ...
                chatInputView.sendButton.isSelected = true
            } else {
                viewmodel.sendInputTypingEndState()
    ...
    open func willSelectItem(button: UIButton?, index: Int) {
        ...
        case 0://相册
            if (self.isOpenHiding == true) {
                ...
                chatInputView.textView.resignFirstResponder()
                layoutInputView(offset: kTabbarBottomHeight)
                p2pChatDelegate?.buttonClikedItem(type: .image)
            }
        case 1://礼物
            if (self.isOpenHiding == true) {
                p2pChatDelegate?.openHidingAction()
                ...
                chatInputView.textView.resignFirstResponder()
                layoutInputView(offset: kTabbarBottomHeight)
                if NEKitChatConfig.shared.kAppType == 0 {//礼物
                    p2pChatDelegate?.buttonClikedItem(type: .gift)
                } else {
                    //A面常用语
                    p2pChatDelegate?.buttonClikedItem(type: .expressions)
                }
            }
        case 2://视频语音通话
            if (self.isOpenHiding == true) {
                ...
                chatInputView.textView.resignFirstResponder()
                layoutInputView(offset: kTabbarBottomHeight)
                p2pChatDelegate?.buttonClikedItem(type: .call)
            }
        case 3://常用语
            if (self.isOpenHiding == true) {
                p2pChatDelegate?.openHidingAction()
        ...
        case 4://更多
            chatInputView.textView.resignFirstResponder()
            layoutInputView(offset: bottomMoreViewHeight + kTabbarBottomHeight)
            scrollTableViewToBottom()
        case 10://语音
            if (self.isOpenHiding == true) {
                p2pChatDelegate?.openHidingAction()
            } else {
                layoutInputView(offset: !chatInputView.voiceButton.isSelected ? kTabbarBottomHeight + textHeight : kTabbarBottomHeight)
                scrollTableViewToBottom()
            }
        case 11://发送文本
            if (self.isOpenHiding == true) {
                ...
                p2pChatDelegate?.buttonClikedItem(type: .text)
            }
        case 12://表情
            chatInputView.textView.resignFirstResponder()
            layoutInputView(offset: bottomExanpndHeight + kTabbarBottomHeight)
            scrollTableViewToBottom()
        default:
            print("其他点击")
        }
    ...
    open func sendMediaMessage(didFinishPickingMediaWithInfo image: UIImage?) {
        ...
        if self.isOpenHiding == true {
            ...
            //发送图片消息
            p2pChatDelegate?.sendMessage(message: MessageUtils.imageMessage(image: img, session: viewmodel.session, myUserID: myUserID), type: VerifyMessageType.image.rawValue)
        }
        //            if isFile == true,
    ...
    private func startPlay(cell: ChatAudioCellProtocol?, model: MessageAudioModel?, audio: NIMAudioObject, isSend: Bool) {
    ...
    public func linkStopPlay() {
        playingUrl = ""
        p2pChatDelegate?.stopAudio()
        playingCell?.stopAnimation(byRight: playingModel?.message?.isOutgoingMsg ?? true)
        playingModel = nil
    }
    
    @objc func audioPlayCompletionNotifiction(_ notification: NSNotification) {
    ...
    open func recordAudio(_ filePath: String?, didCompletedWithError error: Error?) {
        ...
        if dur > 1 {
            if self.isOpenHiding == true {
                ...
                //回调接口校验当前发送消息
                p2pChatDelegate?.sendMessage(message: MessageUtils.audioMessage(filePath: fp, session: viewmodel.session, myUserID: myUserID), type: VerifyMessageType.audio.rawValue)
            }
    ...
    open func expandMoreAction() {
        self.chatInputView.isOpenHiding = self.isOpenHiding
    ...
    open func didTapMessage(_ cell: UITableViewCell?, _ model: MessageContentModel?, _ replyIndex: Int? = nil) {
        if model?.type == .audio {
            ...
            if let url = audio.url, (url.contains(NEKitChatConfig.shared.imageURL) == true) {
                ...
                if url == playingUrl {
                    if let isPalying = p2pChatDelegate?.isPlayingAudio(), isPalying == true {
                        ...
                        // 播放音频
                        playingUrl = url
                        playingCell = cell as? ChatAudioCellProtocol
                        p2pChatDelegate?.playAudio(url: url)
                        playingCell?.startAnimation(byRight: isSend)
                    }
                } else {
                    linkStopPlay()
                ...
                startPlay(cell: cell as? ChatAudioCellProtocol, model: model as? MessageAudioModel, audio: audio, isSend: isSend)
            }
            ...
            Router.shared.use(CallViewRouter, parameters: param)
        } else if model?.type == .custom, let object = model?.message?.messageObject as? NIMCustomObject {
            if let obj1 = object.attachment as? IMCustomAttachment {
                let type = obj1.type
                let attachmentModel = IMCustomDataModel(obj1.data)
                if type == 100 {
                    if attachmentModel.url.count > 0 {
                        let showController = PhotoBrowserController(
                            urls: ChatMessageHelper.getUrls(messages: viewmodel.messages),
                    ...
                    if attachmentModel.url == playingUrl {
                        if let isPalying = p2pChatDelegate?.isPlayingAudio(), isPalying == true {
                            linkStopPlay()
                        } else {
                            // 播放音频
                            playingUrl = attachmentModel.url
                            playingCell = cell as? ChatAudioCellProtocol
                            p2pChatDelegate?.playAudio(url: attachmentModel.url)
                            playingCell?.startAnimation(byRight: isSend)
                        }
                    } else {
                        linkStopPlay()
...
extension ChatViewController: ChatBaseCellDelegate {
    ...
    open func didTapAvatarView(_ cell: UITableViewCell, _ model: MessageContentModel?) {
        ...
        if let msg = model?.message {
            //isOutgoingMsg为True,表示是自己发出去的消息。消息在右边，否则在左边
            p2pChatDelegate?.tapUserHeader(isLeft: !msg.isOutgoingMsg)
        }
    }
