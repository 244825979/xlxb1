...
@objc
public enum IMListType: Int {
    case conversation = 0 //默认
    case dashan //搭讪列表
    case friend //好友列表
}
...
open class ConversationViewModel: NSObject, ConversationRepoDelegate,
    ...
    public var imListType :IMListType = .conversation //列表类型
    ...
    open func fetchServerSessions(option: NIMFetchServerSessionOption,
        ...
        repo.getSessionList { error, conversaitonList in
            DispatchQueue.main.async {
                ...
                // 可在此处对会话列表进行过滤
                if weakSelf?.imListType == .friend {//亲密度列表
                    //过滤掉小于2级亲密度的用户
                    weakSelf?.conversationListArray = weakSelf?.conversationListArray?.filter { model in
                        if let recentSession = model.recentSession, let sid = recentSession.session?.sessionId {
                ...
                } else if weakSelf?.imListType == .dashan {
                    let myUserID = UserDefaults.standard.value(forKey: "userinfo_user_id") as? String ?? ""
                    //搭讪列表仅显示搭讪用户，筛选掉除了搭讪以为的数据
                    let dashanList = UserDefaults.standard.value(forKey: "userinfo_dashan_list_" + myUserID) as? [String] ?? []
                    weakSelf?.conversationListArray = weakSelf?.filterConversations(conversations: conversaitonList ?? [], sessionIds: dashanList)
                } else {//默认会话列表，需要过滤搭讪列表数据、小助手列表数据、系统通知、活动通知
                    let myUserID = UserDefaults.standard.value(forKey: "userinfo_user_id") as? String ?? ""
                    let dashanList = UserDefaults.standard.value(forKey: "userinfo_dashan_list_" + myUserID) as? [String] ?? []
                    ...
                    weakSelf?.conversationListArray = weakSelf?.filterConversationsExcluding(conversations: conversaitonList ?? [], excludeSessionIds: userList)
                }
    ...
    //自定义的筛选方法，会话列表数组与用户ID数组的数据进行匹配，过滤掉会话列表不包含ID数组的数据，比如搭讪列表
    func filterConversations(conversations: [ConversationListModel],
                             sessionIds: [String]) -> [ConversationListModel] {
        // 将字符串数组转为Set提升查询性能（O(1)复杂度）
        let sessionIdSet = Set(sessionIds)
        return conversations.filter { conversation in
            // 使用KeyPath链访问深层属性：recentSession→session→sessionId
            let currentSessionId = conversation.recentSession?.session?.sessionId
            conversation.recentSession?.session
            return sessionIdSet.contains(currentSessionId ?? "")
        }
    }
    //自定义的筛选方法，会话列表过滤掉包含用户ID数组的用户，比如会话列表过滤掉匹配用户
    func filterConversationsExcluding(conversations: [ConversationListModel],
                                      excludeSessionIds: [String]
    ) -> [ConversationListModel] {
        return conversations.filter { conversation in
            let sessionId = conversation.recentSession?.session?.sessionId
            return !excludeSessionIds.contains(sessionId ?? "")
        }
    }
            ...
            for i in 0 ..< conversationListArray!.count {
                ...
                if targetId == listModel.recentSession?.session?.sessionId {
                    ...
                    if recentSession.session?.sessionType == .P2P {
                        ...
                        //搭讪会话列表停留时，搭讪列表的会话进行调整了，判断当前会话是否还属于搭讪会话，不属于就移除
                        if weakSelf?.imListType == .dashan {
                            if let localExt = recentSession.localExt {
                                let conversationType = localExt["conversation_type"] as? String ?? ""
